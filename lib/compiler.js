var is_server = !this.top

var compFn = (function(settings) {
  ...
  function re(r) {
    var src = r.source.replace(/\{/g, '\\' + B[0]).replace(/\}/g, '\\' + B[1])
    return RegExp(src, r.global ? 'g' : '')
  }

  var bracket = settings.brackets || '{ }',
      B = bracket.split(' '),
      LINE_TAG = /^<([\w\-]+)>(.*)<\/\1>/gim,
      QUOTE = re(/=({[^}]+})([\s\/\>])/g),
      BOOLEAN = re(/([\w\-]+)=["']({[^}]+})["']/g),
  ...
  function compileHTML(html, opts, type) {
    ...
    // foo={ bar } --> foo="{ bar }"
    html = html.replace(QUOTE, '="$1"$2')

    // alter special attribute names
    html = html.replace(BOOLEAN, function(full, name, expr) {
      name = name.toLowerCase()

      // <img src> --> <img data-src>
      if (name == 'src') name = 'data-' + name
    ...
    lines.forEach(function(line, i) {
      ...
      if (l[0] != '}' && l.indexOf('(') > 0 && l.indexOf('function') == -1) {
        ...
        if (m && !/^(if|while|switch|for)$/.test(m[2])) {
          ...
          // foo() { }
          if (end[0] == '}') {
            lines[i] += ' ' + l.slice(m[0].length - 1, -1) + '; this.update() }.bind(this)'
  ...
  function compile(src, opts) {
    ...
    return src.replace(CUSTOM_TAG, function(_, tagName, html, js) {
      ...
      var prefix = opts.cjs ? 'var riot = require(\'riot\');\n' : ''
      return prefix + mktag(tagName, compileHTML(html, opts, type), compileJS(js, opts, type))
    })
...
if (is_server) {
  module.exports = compFn
} else {
  compFn(riot.settings)
}
