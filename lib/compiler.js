...
;(function(is_server) {
  ...
  */
  ...
  var QUOTE = /=({[^}]+})([\s\/\>])/g,
      SET_ATTR = /([\w\-]+)=(["'])([^\2]+?)\2/g,
      EXPR = /{\s*([^}]+)\s*}/g,
      NESTED_TAG = /<\/?([\w\-]+)(\s+[\w\-]+|\s+[\w\-]+=('[^\']*'|"[^\"]*"|{[^}]*}|\S+))*\s*>/g,
      CUSTOM_TAG = /^<<<([\w\-]+)>>>([^\x00]*?)<<<\/\1>>>/gim,
  ...
  function compile(src, opts) {
    ...
    // analyze the depth of the node and mark root nodes
    // <my-tag></my-tag> -> <<<my-tag>>><<</my-tag>>>
    var depth = 0
    src = src.replace(riot.util.brackets(NESTED_TAG), function(m, tagName) {
      return (VOID_TAGS.indexOf(tagName.toLowerCase()) != -1) ? m
             : ('/' == m[1] ? --depth : depth++) ? m
             : m.replace(/^<|>$/g, '$&$&$&') // root nodes (depth == 0)
    })

    return src.replace(CUSTOM_TAG, function(_, tagName, src) {
      var html = '', js = '', type = opts.type, style = '', styleType = 'css'

      // we consider the part after the last HTML tag as a script
      // except when it's a single line
      if (!/\n/.test(src)) html = src
      else js = src.replace(/^([^\x00]*[\w\/}]>$)/m, function(m) {
        html = m
        return ''
      })
