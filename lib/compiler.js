;(function(is_server) {
  ...
  if (typeof riot == "undefined") global.riot = { settings: {} }

  var bracket = riot.settings.brackets || '{ }',
      B = bracket.split(' '),
      LINE_TAG = /^<([\w\-]+)>(.*)<\/\1>/gim,
      QUOTE = re(/=({[^}]+})([\s\/\>])/g),
      BOOLEAN = re(/([\w\-]+)=["']({[^}]+})["']/g),
  ...
  function mktag(name, html, js, opts) {
    if (opts.cjs) {
  ...
  function compile(src, opts) {
    ...
    src = src.replace(LINE_TAG, function(_, tagName, html) {
      return mktag(tagName, compileHTML(html, opts), '', opts)
    })
    ...
    return src.replace(CUSTOM_TAG, function(_, tagName, html, js) {
      ...
      return mktag(tagName, compileHTML(html, opts, type), compileJS(js, opts, type), opts)
    })
