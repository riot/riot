...
var brackets = (function(b, orig) {
  ...
  return function(x) {
    return (
      ...
      // string: set new brackets
      x[0] ? b = x.split(' ') :

      // regexp: rewrite regexp with current brackets
      x.test ? RegExp(x.source
                        .replace(orig[0], b[0].replace(/(?=.)/g, '\\'))
                        .replace(orig[1], b[1].replace(/(?=.)/g, '\\')),
                      x.global ? 'g' : '') :
