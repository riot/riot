  ...
  function expr(s, n) {
    ...
    return /^\s*[\w- "']+ *:/.test(s)
      ...
      ? '[' + s.replace(/\W*([\w- ]+)\W*:([^,]+)/g, function(_, k, v) {
          ...
          // wrap every part of conditional to support undefined vars
...
;(function(is_node) {
  if (is_node) {
    return module.exports = tmplFn
  }
  tmpl = tmplFn(riot.settings)
})(!this.top)
