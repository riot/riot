...
var tmpl = (function (/*DEBUG*/) {
  ...
  function wrapTest(expr, wrap) {

    // First entry: wrap == 2 -- nonull == false
    // Recursive:   wrap == 0 -- nonull == false
      ...
      if (JS_FALSY[mvar])
        ...
        while (!expr.search(JS_VARCHAIN)) {         // while search === 0
          ...
          if ((br = gre.$1)) {                      // js bracket? prefix enclosed vars
            ...
            mvar += br +                            // concat left brackets, wrap inner
              wrapTest(expr.slice(i, j))            //  expr, including right bracket
                                                    //  w/recursion
        ...
        ss.push(                                    // protect this (one) variable in
          wrap ?                                    // their own block, if necessary
