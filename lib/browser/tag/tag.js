function Tag(impl, conf, innerHTML) {
  ...
  impl.tmpl = impl.tmpl.replace(/(<\/?)select( |>)/gim, '$1riot-temp$2')
  ...
  if (dom.innerHTML && !/tbody/.test(tagName) && !/tr/.test(tagName))
    // replace all the yield tags with the tag inner html
    dom.innerHTML = replaceYield(dom.innerHTML, innerHTML)

  dom.innerHTML = dom.innerHTML.replace(/(<\/?)riot-temp( |>)/gim, '$1select$2')
