...
function initChildTag(child, opts, innerHTML, parent, skipName) {
  var tag = new Tag(child, opts, innerHTML),
    tagName = opts.tagName || getTagName(opts.root, skipName),
...
function unmountAll(expressions) {
  ...
  for (i = 0; i < expl; i++) {
    expr = expressions[i]
    if (expr.isTag && expr.tag) expr.tag.unmount(true)
    if (expr.unmount) expr.unmount()
  }
}
 ...
 */
function arrayishAdd(obj, key, value) {
  var dest = obj[key]
