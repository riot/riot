 ...
 * @param { Object } params - tag params
 *   @param { HTMLElement } root - tag DOM selector or DOM node/s
 *   @param { String }      name - name/id of the new riot tag
 *   @param { String }      tmpl - tag template
 *   @param { String }      css - custom tag css
 *   @param { String }      attrs - root tag attributes
 *   @param { Function }    onCreate - user function called
 * @param { Object } opts - tag logic
 * @returns { Tag|Object } new riot tag instance if a root tag is provided /
 *                         the tag implementation object if no root is provided
 */
export function Tag({root, name, tmpl, css, attrs, onCreate}, opts) {
  if (!__TAG_IMPL[name]) tag(name, tmpl, css, attrs, onCreate)
  // automount only if root is defined
  return root ? mountTo(root, name, opts) : __TAG_IMPL[name]
}
...
export function mount(selector, tagName, opts) {
  ...
  function pushTagsTo(root) {
    if (root.tagName) {
      let riotTag = getAttr(root, RIOT_TAG_IS) || getAttr(root, RIOT_TAG)
...
export function update() {
  return each(__VIRTUAL_DOM, tag => tag.update())
}
