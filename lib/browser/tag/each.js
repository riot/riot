...
function unmountRedundant(items, tags, tagName) {
  ...
  while (i > j) {
    remove.apply(tags[i], [tags, i, tagName])
  }
}
 ...
 * @param   { String } tagName - key used to identify the type of tag
 */
function remove(tags, i, tagName) {
  tags.splice(i, 1)
  this.unmount()
  arrayishRemove(this.parent, tagName, this, true)
}
...
export default function _each(dom, parent, expr) {
  ...
  expr.update = function updateEach() {
    ...
    var items = tmpl(expr.val, parent),
      frag = createFrag(),
      olsItemsLength = oldItems.length,
    ...
    each(items, function(item, i) {
      ...
      if (mustCreate) {
        ...
        if (contains(items, oldItems[i])) {
          ...
          // remove the tags that do not belong to the collection anymore
          remove.apply(tag, [tags, i, tagName])
          // noo need to proceed with the other stuff
          return
        }
      }

      // update the tag
      if (!mustCreate) tag.update()

      // cache the original item to use it in the events bound to this node
      // and its children
      defineProperty(tag, '_item', item)
