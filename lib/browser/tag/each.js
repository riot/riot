 ...
 * @param { Node } target - only if inserting, insert before this
 */
function addVirtual(tag, src, target) {
  var el = tag.root.firstChild
  tag._virts = []
  while (el) {
    var sib = el.nextSibling
    if (target)
      src.insertBefore(tag.root, target)
    else
      src.appendChild(el)
    ...
    each(items, function(item, i) {
      ...
      if (!~oldPos || !tag) {
        ...
        // this tag must be insert
          ...
          if (virt)
            addVirtual(tag, root, tags[i].root)
