...
export default function each(dom, parent, expr) {
  ...
  exprObj.update = function updateEach() {
    ...
    //console.log('---- LOOP ----')
    //console.log('ITEMS BEFORE', itemsPrevious)
    //console.log('ITEMS AFTER', itemsUpdated)
    //console.log('HTML BEFORE', root.innerHTML)
    itemsUpdated.forEach((item, i) => {
      ...
      if (!tag) {
        ...
        if (isObject(item))
          defineProperty(item, '_riot_id', tag._riot_id)
