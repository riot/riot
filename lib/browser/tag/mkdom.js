...
function replaceYield(templDom, html) {
  ...
  remove.to.forEach(function(yld) { yld.parentElement.removeChild(yld) })

  var replElem = function(tgt, src) {

    // This is just wrong. But it preserves existing behavior specifically tested for by karma test 'simple html transclusion via <yield> tag'
    // Note that there are TWO spaces before the expression '{ greeting }' in that test (tags/v3.0.0-alpha.9:tests/specs/browser/transclusion.specs.js:30)
    // but the expect() shows only 1 space there
    setInnerHTML(src, src.innerHTML.trim())

    var i = 0, el
    while (el = src.childNodes.item(i++)) {
