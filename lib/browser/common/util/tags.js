...
import { contains, each } from './misc'
...
export function makeVirtual(tag, src, target) {
  var head = document.createTextNode(''),
    ...
    tail = document.createTextNode(''), sib, el

  tag._internal.head = tag.root.insertBefore(head, tag.root.firstChild)
  tag._internal.tail = tag.root.appendChild(tail)

  el = tag._internal.head

  while (el) {
    sib = el.nextSibling
    frag.appendChild(el)
    tag._internal.virts.push(el) // hold for unmounting
    el = sib
  }
